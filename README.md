...
2. [Tasks](#tasks)
3. [Example Domain Overview](#example-domain-overview)
4. [Assumptions and Trade-Offs](#assumptions-and-trade-offs)
5. [Submission Requirements](#submission-requirements)
6. [Assessment Areas](#assessment-areas)
7. [Expected Effort](#expected-effort)
...
- Create an endpoint for searching visits using the following query params:
  ...
  - `page`
  - `pageSize`
- A visit's current status may be updated, however all status transitions must be retained as an immutable audit history.
...
|---|---|---|---|
| 1 | Design API Endpoints | Design the API contract for creating and retrieving visit records. | API contracts for create and read operations are defined. |
| 2 | Implement Create Visit Endpoint — `POST /api/visits` | Implement the endpoint to create a visit record. | The endpoint accepts visit details and stores them. |
| 3 | Implement Read Visit Endpoint — `GET /api/visits/{id}` | Implement the endpoint to retrieve a visit by its Id. | The endpoint returns the stored visit record when found. A suitable response is returned when the record does not exist. |
| 4 | Implement Search Visits Endpoint — `GET /api/visits` | Implement the endpoint that allows visit records to be searched and filtered. | The endpoint supports filtering using documented query parameters, supports pagination, returns matching visit records, and returns paging metadata where appropriate. |
| 5 | Data Storage Setup | Set up the data storage for visit records. | A working data storage solution is in place. |
| 6 | Security Implementation | Implement security measures to protect API endpoints. | API endpoints are secured in a manner consistent with the documented security requirements. |
| 7 | Unit Testing | Write unit tests for business logic. | All business logic is covered by unit tests. |
| 8 | Code Review and Refactoring | Review and refactor code to ensure best practices. | — |
